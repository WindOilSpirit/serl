...
pkill -INT -f "[t]eleop_dashboard" || true
pkill -INT -f "[s]pacemouse_franka_impedance_dashboard" || true
pkill -INT -f "[r]un_dashboard.sh" || true
sleep 2
...
pkill -TERM -f "[t]eleop_dashboard" || true
pkill -TERM -f "[s]pacemouse_franka_impedance_dashboard" || true
pkill -TERM -f "[r]un_dashboard.sh" || true
sleep 2
...
pgrep -af "franka.launch.py|franka_safe_bringup.launch.py|ros2_control_node|controller_manager|serl_cartesian_impedance_controller|ros2 launch spacemouse_franka_teleop_test|spacemouse_franka_teleop.launch.py|teleop_node|pose_action_server|spacemouse_franka_impedance_teleop_server|teleop_dashboard|spacemouse_franka_impedance_dashboard|run_dashboard.sh" || true
