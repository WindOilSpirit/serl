...
def require_existing_file(path: str, flag_name: str) -> str:
    if not path:
        raise ValueError(f"{flag_name} must be set.")
    if not os.path.isfile(path):
        raise FileNotFoundError(f"{flag_name} does not exist: {path}")
    return path


def prepare_checkpoint_dir(path: str, flag_name: str) -> str:
    if not path:
        raise ValueError(f"{flag_name} must be set.")
...
KEYBOARD_PAUSE_KEY = None


def start_pause_listener():
    """Start keyboard pause listener when a keyboard backend is available."""
    global KEYBOARD_PAUSE_KEY
    try:
        from pynput import keyboard

        KEYBOARD_PAUSE_KEY = keyboard.Key.pause
        listener = keyboard.Listener(on_press=pause_callback)
        listener.start()
        return listener
    except Exception as exc:
        reason = str(exc).splitlines()[0]
        print(f"Keyboard pause listener disabled: {type(exc).__name__}: {reason}")
        return None


listener = start_pause_listener()
...
def learner(rng, agent: DrQAgent, replay_buffer, demo_buffer):
    ...
    if FLAGS.checkpoint_period:
        prepare_checkpoint_dir(FLAGS.checkpoint_path, "--checkpoint_path")
...
def main(_):
    ...
    if FLAGS.learner:
        demo_path = require_existing_file(FLAGS.demo_path, "--demo_path")
