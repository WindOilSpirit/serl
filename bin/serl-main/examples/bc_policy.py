...
def require_existing_files(paths, flag_name):
    if not paths:
        raise ValueError(f"{flag_name} must be set.")
    for path in paths:
        if not os.path.isfile(path):
            raise FileNotFoundError(f"{flag_name} entry does not exist: {path}")
    return paths


def require_existing_checkpoint(path, flag_name):
    if not path:
        raise ValueError(f"{flag_name} must be set.")
...
def prepare_checkpoint_dir(path, flag_name):
    if not path:
        raise ValueError(f"{flag_name} must be set.")
...
def main(_):
    ...
    if not FLAGS.eval_checkpoint_step:
        ...
        Training Mode
        """
        demo_paths = require_existing_files(FLAGS.demo_paths, "--demo_paths")
        if FLAGS.save_model:
        ...
        if FLAGS.remove_xy:
            ...
            replay_buffer = populate_data_store_with_z_axis_only(
                replay_buffer, demo_paths
            )
        else:
            replay_buffer = populate_data_store(replay_buffer, demo_paths)
        ...
        Evaluation Mode
        """
        checkpoint_path = require_existing_checkpoint(
