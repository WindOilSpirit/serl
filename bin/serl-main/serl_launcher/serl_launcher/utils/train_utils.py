...
def resolve_resnet10_path(file_name="resnet10_params.pkl", allow_download=True):
    """Resolve the pretrained ResNet-10 weights path used by SERL examples."""
    repo_root = Path(__file__).resolve().parents[3]
    candidates = [
        Path.cwd() / file_name,
        repo_root / ".cache_home" / ".serl" / file_name,
        Path(os.path.expanduser("~/.serl")) / file_name,
    ]
    ...
    download_dir = Path(os.path.expanduser("~/.serl"))
...
def load_resnet10_params(agent, image_keys=("image",), public=True):
    ...
    if not public:  # if github repo is not public, load from local file
        with open(resolve_resnet10_path(file_name, allow_download=False), "rb") as f:
            encoder_params = pkl.load(f)
    else:  # when repo is released, download from url
        file_path = resolve_resnet10_path(file_name, allow_download=True)
